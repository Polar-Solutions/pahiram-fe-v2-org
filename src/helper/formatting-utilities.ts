...
const formatBorrowStatus = (
  status: string | null | undefined
): { formattedStatus: string; badgeClass: string } => {
  // Ensure status is a valid string, defaulting to "Unknown" if it's null or undefined
  if (!status) {
    return {
      formattedStatus: "Unknown",
      badgeClass: "bg-gray-200 text-gray-800 text-center",
    };
  }
...
const checkTransactionStatus = (
  status: string
): { transacStatus: string; canCancel: boolean; canEdit: boolean } => {
  const transacStatus = status;

  // Check if the transaction can be canceled
  const canCancel =
    transacStatus === "PENDING_ENDORSER_APPROVAL" ||
    transacStatus === "PENDING_BORROWING_APPROVAL" ||
    transacStatus === "APPROVED";

  // Check if the transaction can be edited
  const isPendingBorrowingApproval =
    transacStatus === "PENDING_BORROWING_APPROVAL";
  const isPendingEndorserApproval =
    transacStatus === "PENDING_ENDORSER_APPROVAL";
...
const formatBorrowedItemStatus = (
  status: string | null | undefined
): {
  ...
  // Determine the badge class based on the status
  let variant: "default" | "destructive" | "success" | "outline" = "default";
  ...
  if (warningStatuses.includes(status)) {
    ...
    variant = "default";
  } else if (status === "APPROVED" || status === "IN_POSSESSION") {
    variant = "success";
  } else {
    // Unreleased
    variant = "outline";
  ...
  return { formattedStatus, variant };
};
