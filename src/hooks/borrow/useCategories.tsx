...
import { IItemCategory, IItemGroup } from "@/lib/interfaces";
...
export const useCategories = () => {
  ...
  effect(() => {
    async function loadCategories() {
      ...
      if (
        !categories.value.some((cat) => cat.page === pageToLoad) &&
        !isFetchingCategoryPage.value
      ) {
        try {
          ...
          if (itemsCategoriesData) {
            const newCategories: CategoryWithPage[] =
              itemsCategoriesData.categories.map((cat) => ({
                ...cat,
                page: pageToLoad,
              }));
